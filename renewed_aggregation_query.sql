CREATE OR REPLACE TABLE `data-prd-adhoc.temp_ammar.collection_notification_features_sept_2025` AS (
...
vintage_data AS (
  ...
  -- where v.business_date is not null
  CROSS JOIN latest_date
  WHERE v.business_date between '2025-08-01' and CURRENT_DATE()
...
final_dataset AS (
  ...
  LEFT JOIN notification_aggregated notif
    ON summary.lfs_customer_id = notif.lfs_customer_id
    AND summary.deal_reference = notif.deal_reference
)
